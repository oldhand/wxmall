...
try{   
	...
	$supplier_albums = XN_Query::create ( 'Content' ) ->tag("supplier_albums_".$supplierid)	 
				->filter('type','eic','supplier_albums')
	...
	$data = array();
	$key = 1;
	if (count($supplier_albums) > 0)
	{  
		foreach($supplier_albums as $supplier_album_info)
		{  
			$description = $supplier_album_info->my->description;   
			$data[$key]['id'] = $supplier_new_info->id;    
			$data[$key]['description'] = $description;    
			$data[$key]['image'] = $supplier_album_info->my->image;   
			$key ++; 
		} 
	}   
}
